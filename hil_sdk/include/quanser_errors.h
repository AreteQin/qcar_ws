...
enum tag_error
{
    ...
    QERR_CANNOT_SUBTRACT_TIMEOUTS,                                      /* The timeouts cannot be subtracted because the left-hand side is relative and the right hand side is absolute. */
    ...
    QERR_HIL_WRITE_ANALOG_NOT_SUPPORTED,                                /* The hil_write_analog function in C or MATLAB and HIL Write Analog block in QUARC are not supported by this particular card. You might need to change a board specific option to enable it. */  /* The Analog polymorphic instances of the HIL Write VI are not supported by this card. */
    ...
    QERR_HIL_WRITE_PWM_NOT_SUPPORTED,                                   /* The hil_write_pwm function in C or MATLAB and HIL Write PWM block in QUARC are not supported by this particular card. You might need to change a board specific option to enable it. */ /* The PWM polymorphic instances of the HIL Write VI are not supported by this card. */
    ...
    QERR_NO_INTERNAL_USE_LICENSE,                                       /* The "Active during normal simulation" feature is not available. Did you intend to run in real-time using the "Monitor & Tune" button? Due to safety and liability concerns, accessing physical hardare in normal simulation rather than real-time is not enabled. */
    ...
    QERR_THEME_INCOMPATIBLE_WITH_DECOLORIZATION,                        /* The selected colorization theme is incompatible with decolorization. Please choose a different theme. */
    QERR_DYNAMIXEL_FAILED_TO_INITIALIZE,                                /* The Dynamixel OpenManipulator X failed to initialize.  Make sure the Dynamixel OpenManipulator X is plugged in and powered. */
    QERR_DYNAMIXEL_COMMUNICATION_ERROR,                                 /* Could not communicate with Dynamixel OpenManipulator X. */
    QERR_DYNAMIXEL_COULD_NOT_OPEN_DEVICE,                               /* Could not open the Dynamixel OpenManipulator X device. */
    QERR_DYNAMIXEL_COULD_NOT_START_DEVICE,                              /* Could not start the Dynamixel OpenManipulator X control loop. */
    QERR_DYNAMIXEL_COULD_NOT_CREATE_READ_GROUP,                         /* Could not create a read group for the Dynamixel OpenManipulator X. */
    QERR_DYNAMIXEL_COULD_NOT_CREATE_WRITE_GROUP,                        /* Could not create a write group for the Dynamixel OpenManipulator X. */
    QERR_DYNAMIXEL_COULD_NOT_CHANGE_WRITE_GROUP,                        /* Could not change parameters of a write group for the Dynamixel OpenManipulator X. */
    QERR_DYNAMIXEL_INVALID_GOAL_PWM,                                    /* The goal PWM for the Dynamixel OpenManipulator X device is out of range. */
    QERR_DYNAMIXEL_INVALID_GOAL_POSITION,                               /* The goal Position for the Dynamixel OpenManipulator X device is out of range. */
    QERR_DYNAMIXEL_INVALID_GAINS,                                       /* The position PID or feedforward gains for the Dynamixel OpenManipulator X device is out of range. */
    QERR_DYNAMIXEL_INVALID_PROFILE_PARAMS,                              /* The profile velocity or acceleration parameters for the Dynamixel OpenManipulator X device is out of range. */
    QERR_CANNOT_CONNECT_TO_PLC,                                         /* It was not possible to connect to the specified PLC. It is likely unreachable. */
    QERR_FILE_ALREADY_EXISTS,                                           /* The file already exists and cannot be overwritten. Delete the file first. */
    QERR_TOO_MANY_TELEMETRY,                                            /* There are too many telemetry bits set in DSHOT commands. When multiple ESC channels share a single telemetry line, only one channel may perform a telemetry request at one time. */
    QERR_DIGITAL_EXPIRATION_STATE_NOT_UNCHANGED,                        /* This board does not support setting the expiration state of all digital outputs. Set digital outputs which are not controlled by the watchdog to 3 (no change). */
    QERR_VARIABLE_STEP_SOLVERS_NOT_SUPPORTED,                           /* Variable-step solvers are not supported by this block. */
    QERR_INVALID_NEWLINE_SEPARATED_STRING,                              /* The initialization string is invalid. A single string containing items separated by newlines (\n) is expected. */
    QERR_SENSOR_TYPE_NOT_SUPPORTED,                                     /* The specified sensor type is not currently supported. */
    QERR_MISSING_SPINNAKER,                                             /* The Spinnaker SDK does not appear to be installed or is not in the PATH. See the help for the GenICam capture block for the installation requirements. Note that a reboot may be required after installing the SDK. */
    QERR_GENICAM_CANNOT_INITIALIZE_CAMERA,                              /* An error occurred while attempting to initialize the GenICam camera. */
    QERR_GENICAM_CANNOT_GET_FEATURES,                                   /* An error occurred while attempting to get features from the GenICam camera. */
    QERR_GENICAM_CANNOT_READ_NODE,                                      /* An error occurred while attempting to read nodes from the GenICam camera.*/
    QERR_GENICAM_CANNOT_WRITE_NODE,                                     /* An error occurred while attempting to write nodes to the GenICam camera.*/
    QERR_GENICAM_CANNOT_FIND_CAMERA,                                    /* The requested GenICam camera cannot be found. */
    QERR_GENICAM_CANNOT_GET_CAMERA_NODE,                                /* The requested GenICam camera nodemap cannot be acquired. */
    QERR_GENICAM_CANNOT_SET_IMAGE_SIZE,                                 /* Cannot set GenICam camera image to the desired size. */
    QERR_GENICAM_CANNOT_SET_PIXEL_FORMAT,                               /* Cannot set GenICam camera image to the desired pixel format. */
    QERR_GENICAM_CANNOT_SET_ANALOG_CONTROL,                             /* Cannot set GenICam camera analog controls to desired value. */
    QERR_GENICAM_CANNOT_SET_ACQUISITION_NODE,                           /* Cannot set GenICam camera acquisition mode. */
    QERR_GENICAM_CANNOT_GRAB_IMAGE,                                     /* An error occurred while attempting to grab an image using the GenICam camera. */
    QERR_GENICAM_GRAB_IMAGE_TIMEOUT,                                    /* A timeout occurred while attempting to grab an image using the GenICam camera. */
    QERR_GENICAM_CANNOT_CLOSE,                                          /* An error occurred while attempting to close the GenICam camera. */
    QERR_CONTINUOUS_THRESHOLD_EXCEEDS_PEEK,                             /* The specified continuous threshold exceeds the peak threshold. */
    QERR_PEER_CLOSED_CONNECTION,                                        /* The connection was closed by the peer. The stream should be closed. */
    QERR_USB_CANNOT_GET_CONFIG_DESCRIPTOR,                              /* Cannot get the device's USB configuration descriptor. Try power cycle the device or use another USB port.  */
    QERR_USB_CANNOT_CLAIM_INTERFACE,                                    /* Cannot claim the device's USB interface. Try power cycle the device or use another USB port. */
    QERR_USB_CANNOT_INITIALIZE,                                         /* USB library cannot be initialized. Try power cycle the device or use another USB port. */
    QERR_USB_CANNOT_FIND_DEVICE,                                        /* Cannot find the USB device. Make sure the device is connected and powered (if it's a powered device). */
    QERR_USB_CANNOT_OPEN_DEVICE,                                        /* Cannot open the device. Try power cycle the device or use another USB port. */
    QERR_USB_CANNOT_GET_ENDPOINT,                                       /* Cannot get device's USB endpoint. Try power cycle the device or use another USB port. */
    QERR_USB_INCOMPATIBLE_CONFIG_DESCRIPTOR,                            /* The config descriptor is not compatible. Try power cycle the device or use another USB port. */
