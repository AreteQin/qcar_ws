...
int main(int argc, char **argv) //int main(int argc, char * argv[]) <-- this is what showned in quanser doc
{
    ...
    if (csi_front != NULL) {
        result = video_capture_open("video://localhost:3", frame_rate, frame_width, frame_height,
                                    IMAGE_FORMAT_ROW_MAJOR_INTERLEAVED_BGR, IMAGE_DATA_TYPE_UINT8, csi_front, &capture,
                                    NULL, 0);
        if (result >= 0) {
