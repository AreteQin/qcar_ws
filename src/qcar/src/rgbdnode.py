...
class RGBDNode(object):
	def __init__(self):
		super().__init__()
		self.rgbd_color_pub = rospy.Publisher('/qcar/rgbd_color', Image, queue_size=1)
		self.rgbd_depth_pub = rospy.Publisher('/qcar/rgbd_depth', Image, queue_size=1)
		self.bridge = CvBridge()
		
		#Initialize CV sensors
		rgbd = Camera3D(mode='RGB&DEPTH', frame_width_RGB=1280, frame_height_RGB=720)
		#rgbd = Camera3D(mode='RGB&DEPTH', frame_width_RGB=640, frame_height_RGB=480)
		...
		while not rospy.is_shutdown():
			...
			self.process_color_data(self.rgbd_color_pub, cv2.resize(rgbd.image_buffer_RGB, (640, 480)))
			self.process_depth_data(self.rgbd_depth_pub, cv2.resize(rgbd.image_buffer_depth_m, (640, 480)))
